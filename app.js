...
async function toggleDarkMode() {
    ...
    if (storageMode === 'firebase' && firebaseLoaded && db) {
        try {
            ...
            log.warn('DarkMode', 'Could not save preference', { error: error.message });
        }
    }
...
async function loadDarkModePreference() {
    if (storageMode === 'firebase') {
        try {
            ...
            log.warn('DarkMode', 'Could not load preference', { error: error.message });
        }
    }
}
...
async function saveAttendance() {
    ...
    try {
        ...
        hideLoading();
        log.error('Attendance', 'Error saving attendance', { error: error.message });
...
async function exportStudentWisePDF() {
    ...
    if (students.length === 0) {
        hideLoading();
        log.warn('Export', 'Student-wise PDF cancelled: no students found');
...
async function exportSubjectWisePDF() {
    ...
    if (!subjectId) {
        hideLoading();
        log.warn('Export', 'Subject-wise PDF cancelled: no subject selected');
        alert('Please select a subject first.');
        return;
    }
    ...
    if (subjectAttendance.length === 0) {
        hideLoading();
        log.warn('Export', 'Subject-wise PDF cancelled: no records for ' + subjectName);
        alert('No attendance records found for this subject.');
        return;
    }
...
async function exportRollSearchPDF() {
    ...
    if (!searchTerm) {
        hideLoading();
        log.warn('Export', 'Roll-search PDF cancelled: no search term');
    ...
    if (matchingStudents.length === 0) {
        hideLoading();
        log.warn('Export', 'Roll-search PDF cancelled: no matches for "' + searchTerm + '"');
...
async function exportSubjectWiseCSV() {
    ...
    if (!subjectId) {
        hideLoading();
        log.warn('Export', 'CSV cancelled: no subject selected');
        alert('Please select a subject first.');
        return;
    }
    ...
    if (subjectAttendance.length === 0) {
        hideLoading();
        log.warn('Export', 'CSV cancelled: no records for ' + subjectName + (dateFrom || dateTo ? ' in date range' : ''));
